...
function daySelecboxByTagId(val, tagName, limitDayCount, tagId) {
	var selectbox = "<select class='select' id='"+tagId+"'>";
	for (var i=1; i<=limitDayCount; i++) {
		if (i == val) {
			selectbox += "<option value='"+val+"' selected>"+val+"일</option>";
		} else {
...
function conditionSelectBox(val, tagName) {
	...
	if (val == 1) {
		...
		selectbox += "<option value='6'>레어아이템</option>";
	} else if (val == 2) { 
		...
		selectbox += "<option value='4'>할인중</option>";
		selectbox += "<option value='5'>판매중지</option>";
		selectbox += "<option value='6' selected>레어아이템</option>";
	}
	selectbox += "</select>";
	$("#"+tagName).html(selectbox);
}
...
var SELECTBOX = function() {
	this.rareItemListSelectBox = function(val, tagName) {
		var selectbox = "<select class='select' id='sel_itemCode'>";
		selectbox += "<option value=''>▶아이템선택</option>";
		itemService.rareItemList(function(list) {
			for (var i=0; i<list.length; i++) {
				if (list[i].itemCode == val) {
					selectbox += "<option value="+list[i].itemCode+" selected>"+ list[i].productName +"</option>";
				} else {
					selectbox += "<option value="+list[i].itemCode+">"+ list[i].productName +"</option>";
				}
			}
			selectbox += "</select>";
	...
	this.recommendSortSelectBox = function(val, tagName) {
		var selectbox = "<select class='select' id='sel_recommendSort' onChange='javascript:fn_search3("+ '"'+"new"+'"'+");'>";
		selectbox += "<option value=''>▶정렬기준</option>";
		if (val == "CURRENT") {
			selectbox += "<option value='CURRENT' selected> - 시작일 빠른순</option>";
			selectbox += "<option value='NEW' > - 시작일 느린순</option>";
		} else if (val == "NEW") { 
			selectbox += "<option value='CURRENT' > - 시작일 빠른순</option>";
			selectbox += "<option value='NEW' selected> - 시작일 느린순</option>";
		}
		selectbox += "</select>"; 
		innerHTML(tagName, selectbox);
	}
